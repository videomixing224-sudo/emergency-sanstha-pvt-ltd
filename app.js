...
<div class="table-wrap"><table class="table"><tr><th>Name</th><th>Type</th><th>User ID</th><th>Mobile</th><th>Father/Husband</th><th>Language</th><th>Status</th><th>Action</th></tr>${rows.map(r=>`<tr><td>${esc(r.name)}</td><td>${r.role}</td><td>${esc(r.login_id||r.mobile)}</td><td>${esc(r.mobile)}</td><td>${esc(r.father_husband)}</td><td>${esc(r.language)}</td><td>${r.status}</td><td><button class="btn secondary" onclick='openCustomerForm(${JSON.stringify(r)})'>Edit</button> <button class="btn secondary danger-btn" onclick='deleteCustomer(${Number(r.id)},${JSON.stringify(r.name)})'>Delete</button></td></tr>`).join("")}</table></div>`;
}
async function deleteCustomer(id,name){
  if(!Number.isInteger(Number(id)) || Number(id)<=0){ alert("Invalid customer ID"); return; }
  if(!confirm(`Kya aap customer "${name}" ko delete karna chahte hain?\n\nSirf cleared/closed loan ya zero outstanding wala customer delete hoga.`)) return;
  try{
    const result=await api(`/api/admin/customers/${Number(id)}`,{method:"DELETE"});
